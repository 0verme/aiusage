...
describe('Grok Build scanner', () => {
  ...
  it('uses real paired usage fields ahead of message text estimates', async () => {
    await session({
      'summary.json': JSON.stringify({ model_id: 'grok-4.5' }),
    ...
    const [result] = (await scanGrokBuildDates([day], root)).get(day)!;
    expect(result).toMatchObject({ inputTokens: 100, outputTokens: 20 });
  });
  ...
  it('reads real Grok Build event field `ts` and counts turns', async () => {
    await session({
      'summary.json': JSON.stringify({
        ...
        created_at: `${day}T04:25:10.098894700Z`,
      }),
      'events.jsonl': [
        JSON.stringify({ ts: `${day}T04:25:30.055Z`, type: 'turn_started', model_id: 'grok-4.5', turn_number: 0 }),
        JSON.stringify({ ts: `${day}T04:41:35.755Z`, type: 'turn_started', model_id: 'grok-4.5', turn_number: 1 }),
        JSON.stringify({ ts: `${day}T04:25:31.411Z`, type: 'first_token' }),
      ].join('\n'),
      'chat_history.jsonl': [
  ...
  it('parses array content and type-as-role without double-counting updates chunks', async () => {
    ...
    await session({
      'summary.json': JSON.stringify({ current_model_id: 'grok-4.5', created_at: `${day}T10:00:00Z` }),
      'events.jsonl': JSON.stringify({ ts: `${day}T10:00:01Z`, type: 'turn_started', model_id: 'grok-4.5' }),
      'chat_history.jsonl': [
      ...
      'updates.jsonl': [
        JSON.stringify({
          timestamp: Math.floor(new Date(`${day}T10:00:01Z`).getTime() / 1000),
        ...
        JSON.stringify({
          timestamp: Math.floor(new Date(`${day}T10:00:02Z`).getTime() / 1000),
          method: 'session/update',
          params: {
  ...
  it('falls back to updates chunks when chat_history is missing', async () => {
    await session({
      'summary.json': JSON.stringify({ current_model_id: 'grok-4.5', created_at: `${day}T11:00:00Z` }),
      'events.jsonl': JSON.stringify({ ts: `${day}T11:00:01Z`, type: 'turn_started' }),
      'updates.jsonl': [
        JSON.stringify({
          timestamp: Math.floor(new Date(`${day}T11:00:01Z`).getTime() / 1000),
          method: 'session/update',
          params: {
        ...
        JSON.stringify({
          timestamp: Math.floor(new Date(`${day}T11:00:02Z`).getTime() / 1000),
          method: 'session/update',
          params: {
  ...
  it('counts cached input from paired usage when present', async () => {
    await session({
      'summary.json': JSON.stringify({ model_id: 'grok-4.5' }),
    ...
    const [result] = (await scanGrokBuildDates([day], root)).get(day)!;
    expect(result).toMatchObject({ inputTokens: 80, outputTokens: 10, cachedInputTokens: 40 });
  });
  ...
  it('counts assistant tool_calls as output even when content is empty', async () => {
    ...
    await session({
      'summary.json': JSON.stringify({ current_model_id: 'grok-4.5', created_at: `${day}T13:00:00Z` }),
      'events.jsonl': JSON.stringify({ ts: `${day}T13:00:01Z`, type: 'turn_started' }),
      'chat_history.jsonl': [
  ...
  it('counts agent_thought_chunk as reasoning without double-counting message text', async () => {
    ...
    await session({
      'summary.json': JSON.stringify({ current_model_id: 'grok-4.5', created_at: `${day}T14:00:00Z` }),
      'events.jsonl': JSON.stringify({ ts: `${day}T14:00:01Z`, type: 'turn_started' }),
      'chat_history.jsonl': [
      ...
      'updates.jsonl': [
        JSON.stringify({
          timestamp: Math.floor(new Date(`${day}T14:00:01Z`).getTime() / 1000),
          method: 'session/update',
          params: {
        ...
        JSON.stringify({
          timestamp: Math.floor(new Date(`${day}T14:00:02Z`).getTime() / 1000),
          method: 'session/update',
          params: {
            update: { sessionUpdate: 'agent_message_chunk', content: { type: 'text', text: reply } },
            _meta: { eventId: 'm1' },
