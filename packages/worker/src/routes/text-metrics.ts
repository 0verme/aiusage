...
import { resolvePublicProjectFilter } from "../utils/privacy.js";
import {
	CACHE_PRESETS,
	corsHeaders,
	jsonError,
...
export async function handleTextTokens(url: URL, env: Env): Promise<Response> {
	...
	if (!unit)
		return jsonError(400, "INVALID_PAYLOAD", "Invalid unit parameter", true);
  ...
  `)
	...
	return withCacheHeaders(
		new Response(formatTokenCount(row?.total_tokens ?? 0, unit), {
			status: 200,
			headers: {
				...corsHeaders(),
...
export function normalizeTokenUnit(value: string | null): TokenUnit | null {
	...
	if (unit === "auto" || unit === "en" || unit === "zh" || unit === "raw")
		return unit;
	return null;
}
